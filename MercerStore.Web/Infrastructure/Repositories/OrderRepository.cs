...
namespace MercerStore.Web.Infrastructure.Repositories
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public async Task<(IEnumerable<Order>, int totalItems)> GetFilteredOrders(OrderFilterRequest request)
        {
        ...
        public async Task<SalesMetricDto> GetSalesMetric()
        {
            var now = DateTime.UtcNow;
            var startOfWeek = now.AddDays(-(int)now.DayOfWeek);
            var startOfMonth = new DateTime(now.Year, now.Month, 1);
            var startOfQuarter = now.AddMonths(-startOfMonth.Month);
