...
namespace MercerStore.Web.Application.Handlers.Reviews.Commands
{
    ...
    public class AddReviewHandler : IRequestHandler<AddReviewCommand, Result<Unit>>
    {
        ...
        public async Task<Result<Unit>> Handle(AddReviewCommand request, CancellationToken cancellationToken)
        {
            var userId = _userIdentifierService.GetCurrentIdentifier();
            var existingReview = await _reviewService.GetReview(request.CreateReviewDto.ProductId, userId);

            if (existingReview != null)
            {
                return Result<Unit>.Failure("Отзыв уже существует");
            }
