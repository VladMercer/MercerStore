...
namespace MercerStore.Web.Application.Handlers.Search.Queries
{
    ...
    public class SearchProductHandler : IRequestHandler<SearchProductQuery, SearchResultDto>
    {
        ...
        public async Task<SearchResultDto> Handle(SearchProductQuery request, CancellationToken cancellationToken)
        {
            var productIndexDto = await _elasticSearchService.SearchProductsAsync(request.Request.Query);
            return await _searchService.SearchProduct(productIndexDto, request.Request);
        }
    }
}
