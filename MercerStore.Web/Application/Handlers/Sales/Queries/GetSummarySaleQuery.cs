...
namespace MercerStore.Web.Application.Handlers.Sales.Queries
{
    ...
    public class GetSummarySaleHandler : IRequestHandler<GetSummarySaleQuery, Result<OfflineSale>>
    {
        ...
        public async Task<Result<OfflineSale>> Handle(GetSummarySaleQuery request, CancellationToken cancellationToken)
        {
            var result = await _saleService.GetSummarySale(request.SaleId);

            if (!result.IsSuccess)
            {
                return Result<OfflineSale>.Failure(result.ErrorMessage);
            }
