...
namespace MercerStore.Web.Application.Handlers.Sales.Commands
{
    ...
    public class CloseSaleHandler : IRequestHandler<CloseSaleCommand, Result<Unit>>
    {
        ...
        public async Task<Result<Unit>> Handle(CloseSaleCommand request, CancellationToken cancellationToken)
        {
            var result = await _saleService.CloseSale(request.SaleId);
            if (!result.IsSuccess)
            {
