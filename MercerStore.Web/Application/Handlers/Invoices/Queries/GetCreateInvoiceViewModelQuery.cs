...
public class GetCreateInvoiceViewModelHandler : IRequestHandler<GetCreateInvoiceViewModelQuery, CreateInvoiceViewModel>
{
    ...
    public GetCreateInvoiceViewModelHandler(IInvoiceService invoiceService,
    ...
    public async Task<CreateInvoiceViewModel> Handle(GetCreateInvoiceViewModelQuery request,
        CancellationToken ct)
    {
        var managerId = _userIdentifierService.GetCurrentIdentifier();
        return await _invoiceService.GetCreateInvoiceViewModel(request.SupplierId, managerId, ct);
    }
}