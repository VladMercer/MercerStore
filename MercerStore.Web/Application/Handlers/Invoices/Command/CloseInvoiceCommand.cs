...
namespace MercerStore.Web.Application.Handlers.Invoices.Command
{
    ...
    public class CloseInvoiceHandler : IRequestHandler<CloseInvoiceCommand, Result<Unit>>
    {
        ...
        public async Task<Result<Unit>> Handle(CloseInvoiceCommand request, CancellationToken cancellationToken)
        {
            var result = await _invoiceService.CloseInvoice(request.InvoiceId, request.Notes);
