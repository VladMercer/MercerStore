...
public class AdminUpdateUserProfileHandler : IRequestHandler<AdminUpdateUserProfileCommand, Result<Unit>>
{
    ...
    public async Task<Result<Unit>> Handle(AdminUpdateUserProfileCommand request, CancellationToken ct)
    {
        var result = await _userService.GetUserById(request.UpdateUserProfileViewModel.Id, ct);
        if (!result.IsSuccess) return Result<Unit>.Failure(result.ErrorMessage);
        var user = result.Data;

        user.UserImgUrl = request.UpdateUserProfileViewModel.UserImgUrl ??
                          "https://localhost:7208/img/default/default_user_image.jpg";
